...
class ShwUr5eMoveClient:
    ...
    def joint_traj_move(self, file = None, is_collect = False):
        ...
        if file is not None:
            ...
            if is_collect:
                client.move_once_by_end(joint_pos[0][:6])
                if joint_pos[0][6]:
            ...
            for i in range(joint_pos.shape[0]):
                client.move_once_by_joint(joint_pos[i][:6], need_time = 1)
                rospy.sleep(0.05)
                if joint_pos[i][6]:
                    gripper.close_gripper()
                else:
    ...
    def end_traj_move(self, file = None, is_collect = False, add_random = False):
        ...
        if file is not None:
            ...
            if is_collect:
                client.move_once_by_end(end_pos[0][:7])
                if end_pos[0][7]:
                    gripper.close_gripper()
                else:
                    gripper.open_gripper()
            ...
            for i in range(end_pos.shape[0]):
                client.move_once_by_end(end_pos[i][:7], need_time = 1)
                rospy.sleep(0.05)
                if end_pos[i][7]:
                    gripper.close_gripper()
                else:
